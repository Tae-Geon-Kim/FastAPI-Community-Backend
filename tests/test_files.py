...
# 유저 생성 & 해당 유저 이름으로 게시판 생성 & 헤더 / 게시판 번호 반환
    ...
    # 유저 가입 및 로그인
    await client.post("/user/uRegister", json={"id": user_id, "password": user_pw})
    login_res = await client.post("/user/login", json={"id": user_id, "password": user_pw})
    token = login_res.json()["data"]["access_token"]
    headers = {"Authorization": f"Bearer {token}"}

    # 게시판 생성
    await client.post("/boards/bRegister", headers=headers, json={"title": TEST_TITLE, "content": TEST_CONTENT})
    
    # 방금 만든 게시판 번호 가져오기
    board_res = await client.get(f"/boards/certainBInfo?user_id={user_id}")
    board_index = board_res.json()["data"][0]["index"]
...
async def test_delete_file_wrong_password(client: AsyncClient):
    ...
    # 정상 업로드
    await client.post(
        "/files/uploadFiles", headers=headers, 
        data={"board_index": str(board_index)}, 
