...
@router.get(
    ...
    - view_count 칼럼 조회수를 기준으로 순서를 매겼을 때 동일값으로 인해 5개가 넘어가도 5개만 출력
    - 1등: 100회 2등: 50회 3,4,5,6등: 40회 --> 조회수 3등 (40회) 게시물 중에서 최근에 생성된 게시글 3개를 출력
...
async def get_board_detail(
    ...
    conn: Connection = Depends(get_db),
    viewer_info: dict = Depends(get_viewer)
):

    return await single_board_info_services(board_index, viewer_info, conn, redis_client)
...
@router.delete(
    ...
    - 유저가 삭제하는 게시판은 DB에 영구적으로 보관되며 파일 데이터는 삭제처리 후 100일 뒤 hard delete 된다.
    """
)
...
@router.post(
    ...
    - 복구를 진행했을 때 게시판에 업로드 가능한 총 파일의 용량 (25MB)을 초과하면 복구는 불가하다.
