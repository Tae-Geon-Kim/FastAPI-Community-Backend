...
<a name="start"></a>
## 🚀 시작하기
본 프로젝트는 Docker를 기반으로 실행 환경이 격리되어 있습니다.

로컬 PC에 Python이나 DB (Postgresql)를 직접 설치할 필요가 없습니다.
...
# 2. 환경변수 설정 (예시 파일을 복사하여 실제 환경변수 파일 생성)
cp .env.example .env

# 3. Docker 컨테이너 실행 (App, DB, Nginx, Redis 자동 빌드 및 백그라운드 실행)
docker compose up -d --build

# 4. 데이터베이스 마이그레이션 (Alembic을 통한 초기 테이블 자동 생성)
...
## 🏗️ 시스템 아키텍처
```text
[ Client ] <--> [ API (Router) ] <--> [ Service Layer ] <--> [ Repository (Raw SQL) ] <--> [ DB ]
                          ^                    ^
                          |                    |
...
### 🔐 JWT 인증 흐름
1. **인증 (Login):** 사용자 로그인 성공 시, `Access Token`(30분)과 `Refresh Token`(30일)을 동시 발급합니다.
2. **API 요청 (Auth):** 클라이언트는 API 요청 시 `Access Token`을 포함하여 서버에 접근 권한을 증명합니다.
3. **토큰 갱신 (Refresh):** `Access Token`이 만료되었을 경우, 클라이언트는 `Refresh Token`을 보내어 새로운 `Access Token`을 안전하게 재발급받습니다.
4. 로그아웃 (Logout):** 로그아웃 시 브라우저에 저장된 토큰(쿠키)을 폐기하여 접근을 차단합니다.
